...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category', 'App']).agg({'Reviews': 'mean', 'Rating': 'mean'}).sort_values(by = 'Reviews', ascending = False).head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    cat = pd.crosstab(index = df2['Category'],
                    columns = 'Total Installed',
                    values = df2['Installs'],
                    aggfunc = 'sum').sort_values('Total Installed', ascending = False).head(5).reset_index()
    X = cat['Category']
    Y = cat['Total Installed']
